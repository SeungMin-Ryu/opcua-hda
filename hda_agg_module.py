...
class HistorySQLite(HistoryStorageInterface):
    ...
    def new_historized_node(self, node_id, period, count=0):
        with self._lock:
            ...
            try:
                _c_new.execute('CREATE TABLE {tn} (_Id INTEGER PRIMARY KEY NOT NULL AUTO_INCREMENT,'
    ...
    def save_node_value(self, node_id, datavalue):
        with self._lock:
            ...
            try:
                _c_sub.execute('INSERT INTO {tn}(ServerTimestamp,SourceTimestamp,StatusCode,Value,VariantType,VariantBinary)'
                               'VALUES (%s, %s, %s, %s, %s, %s)'.format(tn=table)
                               ,(
                                   ...
                                   pymysql.Binary(variant_to_binary(datavalue.Value))
                               )
                              )
            except pymysql.Error as e:
            ...
            if period:
                ...
                execute_sql_delete('SourceTimestamp < %s', (date_limit,))

            if count:
                # ensure that no more than count records are stored for the specified node
                execute_sql_delete('SourceTimestamp =(SELECT * FROM'
                                   '(SELECT CASE WHEN COUNT(*) > %s THEN MIN(SourceTimestamp) ELSE NULL END FROM {tn})'
                                   'A)', (count,))
    ...
    def read_node_history(self, node_id, start, end, nb_values):
        with self._lock:
            ...
            try:
                _c_read.execute('SELECT * FROM {tn} WHERE SourceTimestamp BETWEEN "%s" '
                                'AND "%s" ORDER BY _Id {dir} LIMIT %s'
                                .format(tn=table,dir=order) % (start_time, end_time, limit))
    ...
    def _format_event(self, event):
        ...
        for name in names:
            ...
            placeholders.append('?')
            ev_variant_binaries.append(pymysql.Binary(variant_to_binary(variant)))
