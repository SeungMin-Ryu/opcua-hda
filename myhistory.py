...
class HistorySQLite(HistoryStorageInterface):
    ...
    def __init__(self, path="./history.db"):
        ...
        self._conn1 = pymysql.connect(host='172.21.43.101', user='seungmin', password='fbtmdals12', db='subclient', charset='utf8')
    def new_historized_node(self, node_id, period, count=0):
        with self._lock:
            ...
            try:
                _c_new.execute('CREATE TABLE test (_Id INTEGER PRIMARY KEY NOT NULL,'
    ...
    def save_node_value(self, node_id, datavalue):
        with self._lock:
            ...
            try:
                _c_sub.execute('INSERT INTO test VALUES (NULL, ?, ?, ?, ?, ?, ?)',
                               (
            ...
            def execute_sql_delete(condition, args):
                query = ('DELETE FROM test WHERE ' + condition)
            ...
            if count:
                ...
                execute_sql_delete('SourceTimestamp = (SELECT CASE WHEN COUNT(*) > ? '
                                   'THEN MIN(SourceTimestamp) ELSE NULL END FROM test)', (count,))

    def read_node_history(self, node_id, start, end, nb_values):
        with self._lock:
            _c_read = self._conn1.cursor()
            ...
            try:
                _c_read.execute('SELECT * FROM test1 WHERE SourceTimestamp BETWEEN "%s" AND "%s" ORDER BY _Id LIMIT %s'
                                           %(start_time,end_time,limit))
                # rebuild the data value object

                for row in _c_read.fetchall():
    ...
    def new_historized_event(self, source_id, evtypes, period, count=0):
        with self._lock:
            ...
            try:
                _c_new.execute(
                    'CREATE TABLE test (_Id INTEGER PRIMARY KEY NOT NULL, _Timestamp TIMESTAMP, _EventTypeName TEXT, {co})'
                    .format(co=columns))
    ...
    def save_event(self, event):
        with self._lock:
            ...
            try:
                _c_sub.execute(
                    'INSERT INTO test ("_Id", "_Timestamp", "_EventTypeName", {co}) VALUES (NULL, "{ts}", "{et}", {pl})'
                    .format(co=columns, ts=event.Time, et=event_type, pl=placeholders), evtup)
            ...
            if period:
                ...
                try:
                    _c_sub.execute('DELETE FROM test WHERE Time < ?',
        ...
        with self._lock:
            ...
            try:
                for row in _c_read.execute(
                        'SELECT "_Timestamp", {cl} FROM test WHERE "_Timestamp" BETWEEN ? AND ? ORDER BY "_Id" {dir} LIMIT ?'
                        .format(cl=clauses_str, dir=order), (start_time, end_time, limit)):
